...
function Card({
  ...
  return (
    <div className={`${className ? className : " px-[8.9px] pt-[8.89px] "} md:max-w-[300px] bg-[#FFF] rounded-[12px] md:px-[15px] flex flex-col gap-[20px] md:pt-[15px] md:pb-[25px] w-full`}>
      ...
      {!cardInfo && (
        <div className="text-[#000] flex flex-col md:flex-row md:flex gap-[19px] md:items-center md:text-[14px] font-semibold leading-[20px]">
          <p className="max-w-[179px] text-[12px] md:text-[14px] leading-[16px] md:leading-[20px] w-full">{title}</p>
          <p className="leading-[11.889px] self-end md:self-auto mb-[12px] md:mb-0 md:leading-[20px]">{price}</p>
        </div>
      )}

      {cardInfo && (
        <div className={`${classNamePb ? classNamePb : "pb-[12.53px]"} flex md:pb-0 items-center justify-between`}>
          <div className={`${className2 ? className2 : "gap-[6.4px]"} flex md:gap-[12px] items-center md:max-w-[163px] md:w-full`}>
            <img className={`${logoClassName ? logoClassName : "max-w-[21.3px] max-h-[21.3px]"} w-full md:max-w-[40px] md:max-h-[40px]`} src={logo} />
            <div className="flex flex-col gap-[2px]">
              <p className={`${titleClassName ? titleClassName : "text-[8px] leading-[10.667px]"} text-[#000] leading-[10.667px] md:text-[14px] font-semibold md:leading-[20px]`}>
                {title}
              </p>
              <p className={`${typeClassName ? typeClassName : "leading-[8.533px] text-[8px]"} text-[#808191] font-normal  md:leading-[16px]  md:text-[12px]`}>
                {type}
              </p>
            </div>
          </div>
          <p className={`${priceClassName ? priceClassName : "text-[8px] leading-[10.667px]"} md:text-[14px] font-semibold md:leading-[20px] text-[#000]`}>
            {price}
          </p>
        </div>
      )}
